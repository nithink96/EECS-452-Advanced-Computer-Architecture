...
md_addr_t* branch_result( md_addr_t baddr)
{
	int index = 0;
	index = hist[0];
			if(hist_table[index] > 0) return 1;
			else return 0;
...
void update_predictor(md_addr_t baddr, int branch_taken, int pred_taken)
{
	...
	index = hist_table[0];
			if(pred_taken)
			{
				if(hist_table[index]<1)
					 hist_table[index]++;
				hist[0] = (index<<1+1)&7;
			}
			else
			{
				if(hist_table[index]> 0)
					hist_table[index]--;
				hist[0] = (index<<1)&7;
			}
...
//	printf("%f(",pbranch);printf(")");
	...
	stat_reg_counter(sdb,"3 bit GAg predictor", " Number of correct predictions",&correct_pred,0,NULL);
