...
/* register simulator-specific options */
	...
	unsigned int size = 1024*8;
	unsigned int hist_table[1024*8];
...
md_addr_t* branch_result(md_addr_t baddr)
{
	...
	int temp = PRED_HASH(baddr);
			index = hist[temp];
			if(hist_table[index + 8 * temp] > 0 ) return 1;
			else return 0;
...
void update_predictor(md_addr_t baddr, int branch_taken, int pred_taken)
{
	...
	temp = PRED_HASH(baddr)
			index = hist_temp[temp];
			if(pred_taken)
			{
				if(hist_table[temp*8+index]<1)
					hist_table[temp*8 +index]++;
				hist[temp] = (index<<1 +1) & 7;
			}
			else
			{
				if(predicotr.hist_table[temp*8+ index] > 0)
					table[temp*8+index]--;
				hist[temp] = (index<<1 + 1) & 7;
			}
...
//	printf("%f(",pbranch);printf(")");
	...
	stat_reg_counter(sdb,"3 bit GAp predictor", " Number of correct predictions",&correct_pred,0,NULL);
...
//	fprintf(stderr, "Percentage of Conditional Branches\n",cond_branch/branch_counter*100); 
	if((MD_OP_FLAGS(op) & (F_CTRL|F_COND))==(F_CTRL|F_COND))
	{
		...
		if(predictor)
		{
			taken = branch_result(predictor, regs.regs_PC);
			update_predictor(predictor, regs.regs_PC,branch_taken,taken);
		}
	} 
