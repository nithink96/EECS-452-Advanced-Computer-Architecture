...
static counter_t sim_num_refs,branch_counter,cond_branch= 0;
#define PRED_HASH(ADDR) \ ((((ADDR)^19)^((ADDR)>>MD_BR_SHIFT))&(1024-1))
/* maximum number of inst's to execute */
static unsigned int max_insts,correct_pred = 0;
/* register simulator-specific options */

	
	unsigned int size;
	unsigned int hist_table[1024];
	unsigned int hist[8];
 ...
 void create_predictor()
{
	...
	{	
		predictor.hist_table[i] = temp;
		temp = 3 - temp;
	}		
	
}

md_addr_t* branch_result( md_addr_t baddr)
{
	int index = 0;

	if(predictor.hist_table[PRED_HASH(baddr)]>=2) return 1;
	else return 0;

}


void update_predictor(md_addr_t baddr, int branch_taken, int pred_taken)
{
	if(pred_taken == branch_taken)
		correct_pred++;
	int index,temp = 0;
	 temp = PRED_HASH(baddr);
	if(pred_taken)
	{

		if(predictor.hist_table[temp]<3) predictor_hist_table[temp]++;
	}
		else if(predictor.hist_table[temp] > 1)
		predictor.hist_table[temp]--;
...
"rather than speed.\n"
  ...
  opt_reg_uint(odb, "-max:inst", "maximum number of inst's to execute",
	...
	//opt_reg_string(odb,"-branchpred", "Branch Predictor", &predictor_type, "branchpred1bit", TRUE, NULL);
}
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_refs",
		   ...
		   &sim_num_refs, 0, NULL);
//	pbranch = (branch_counter/sim_num_insn)*100.0;
//	pcondbranch = (cond_branch/branch_counter)*100.0;
	stat_reg_counter(sdb,"Branch_counter","Total number of Branches Executed",&branch_counter,0,NULL);
//	printf("%f(",pbranch);printf(")");
	stat_reg_counter(sdb,"Conditional_Branch_Counter","Total number of Conditional Branches Executed",&cond_branch,0,NULL);
		stat_reg_counter(sdb,"2 bit predictor", " Number of correct predictions",&correct_pred,0,NULL);

//	printf("%f(",pcondbranch);printf(")");
...
#include "machine.def"
	...
	if(MD_OP_FLAGS(op)&F_COND)
		cond_branch++;
//	fprintf(stderr, "Percentage of Branch instructions\n",branch_counter/sim_num_insn*100);
//	fprintf(stderr, "Percentage of Conditional Branches\n",cond_branch/branch_counter*100); 
	if((MD_OP_FLAGS(op) & (F_CTRL|F_COND))==(F_CTRL|F_COND))
	{
		int taken = 0;
		int branch_taken = (regs.regs_NPC!=(regs.regs_PC + sizeof(md_inst_t)));
		if(predictor)
		{
			taken = branch_result(regs.regs_PC);
			update_predictor(regs.regs_PC,branch_taken,taken);
		}
	} 
