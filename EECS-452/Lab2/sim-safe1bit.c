...
{
//	struct branchpred_values *predictor; 
	int temp;
	//predictor.class = class;
...
{
	int index = 0;
	if(hist_table[PRED_HASH(baddr)]>=1) return 1;
                        else return 0;
}
...
void update_predictor( md_addr_t baddr, int branch_taken, int pred_taken)
{
	...
	temp = PRED_HASH(baddr);
			if(pred_taken) hist_table[temp] = 1;
			else hist_table[temp] = 0;
}	
...
//	printf("%f(",pbranch);printf(")");
	stat_reg_counter(sdb,"Conditional_Branch_Counter","Total number of Conditional Branches Executed",&cond_branch,0,NULL);
		stat_reg_counter(sdb,"1 bit predictor", " Number of correct predictions",&correct_pred,0,NULL);
