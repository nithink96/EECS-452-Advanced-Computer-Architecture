...
static counter_t sim_num_refs,branch_counter,cond_branch,correct_pred= 0;
#define PRED_HASH(ADDR) ((((ADDR)>>19)^((ADDR)>>MD_BR_SHIFT))&(1024-1))
/* maximum number of inst's to execute */
static unsigned int max_insts= 0;
...
{
	int index = PRED_HASH(baddr);

		
			int checker = hist_table[index][2]*4 + hist_table[index][1]*2+ hist_table[index][0];
			return hist[checker];
...
void update_predictor( md_addr_t baddr, int branch_taken, int pred_taken)
{
	if(!!pred_taken == !!branch_taken)
		correct_pred++;
	int index[3];int temp = PRED_HASH(baddr);		
	int i;


		for(i = 0;i<3;i++)
			index[i] = hist_table[temp][i];

			hist_table[temp][2] = hist_table[temp][1];
                        hist_table[temp][1] = hist_table[temp][0];
			if(pred_taken)	
				hist_table[temp][0] = 1;
			else
				hist_table[temp][0] = 0;
			...
			if(pred_taken != branch_taken)
			{
				hist[index[2]*4+index[1]*2+index[0]] = 1 - hist[index[2]*4 + index[1]*2+index[0]];
			}
...
//	printf("%f(",pcondbranch);printf(")");
	stat_reg_counter(sdb,"3 bit PAp predictor", " Number of correct predictions",&correct_pred,0,NULL);
